...
import datetime
import random
from utils import print_confusion_matrixes


class ModelTrainer():
    def __init__(self, task: int, model: str, run_name: str, data_percentage: float, use_token_type_ids: bool,
                 opimizer_config, tokenizer_config, languages, do_hyperparameter_search=False, **args):
        self.task = task
    ...
    def to_dataset(self, data, stride=0):
        ...
        if self.use_token_type_ids:
            return Dataset.from_dict({'labels': labels, 'token_type_ids': token_type_ids, 'input_ids': input_ids,
    ...
    def compute_metrics_generator(self):
        def metrics(pred):
            ...
            if self.task == 1:
                ...
                print_confusion_matrixes(cm, self.id_2_label)
    ...
    def run_training(self):
        ...
        def model_init():
            return AutoModelForTokenClassification.from_pretrained(self.model_checkpoint,
    ...
    def run_hyperparameter_search(self, trainer):
        ...
        result = trainer.hyperparameter_search(direction="maximize", n_trials=200, hp_space=my_hp_space,
...
if __name__ == "__main__":
    trainer = ModelTrainer(task=2, model="dbmdz/bert-base-italian-xxl-uncased", run_name="optim", data_percentage=0.1,
                           use_token_type_ids=True, opimizer_config={"adafactor": False, "num_train_epochs": 3},
                           tokenizer_config={"strip_accent": True, "add_prefix_space": False}, languages=["it"],
