...
def load_from_zip(data_zip: str, data_set: str, lang: str):
    """
    ...
    with ZipFile(data_zip, 'r') as zf:  # load ground truth
        ...
        for i, gt_tsv_file in enumerate(tqdm(gt_tsv_files), 1):
            ...
            with io.TextIOWrapper(zf.open(gt_tsv_file), encoding="utf-8") as f:
                ...
                labels = [map_label(row[1]) for row in rows]
...
# def load(data_zip: str, data_set: str, lang: str):
#     path = f"{data_zip}_{data_set}_{lang}.pickle"
...
#         data  = load_from_zip(data_zip, data_set, lang)
...
def transform_to_language_model_file(data_zip: str,data_set:str, lang: str,result_path):
    data = load(data_zip,data_set,lang)
...
if __name__ =="__main__":
    ...
    data = load("data/leipzig_aug_de.zip","aug","de")
